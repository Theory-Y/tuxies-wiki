...
:::info
This guide provides a quickstart using `kitty` — from the config file itself, to theming, transparency, and window sizing. `kitty` is our recommended terminal: it scrolls ==smoothly== on touchpads, where most terminals jump whole lines at a time.
:::
