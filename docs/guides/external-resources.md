---
title: External Resources
createTime: 2026/05/24 08:29
permalink: /guides/external-resources/
tags:
  - Beginner
  - Fixes
sticky: 3
contributors:
  - aier
---

## Fixes

### Gnome

- [Touchpad scrolling sensitivity fix.](https://github.com/shivasai573/touchpad-sensitivity-tweak) _(Guide on GitHub by shivasai573)_

## Features

### Facial recognition

:::tip We suggest [Gaze](https://github.com/GunduLabs/gaze) over Howdy. It has a GUI, friendly setup, and supports both IR and normal camera.
:::

- [Gaze](https://gaze.gundulabs.com/) _(See the [GitHub repo](https://github.com/GunduLabs/gaze) for installation)_
- [Facial Recognition With Howdy](https://github.com/boltgolt/howdy?tab=readme-ov-file) _(Official documentation from Howdy)_
- [Getting Howdy to Run on Fedora](https://mtillmann.blog/posts/howdy-on-fedora-43.html) _(Martin's Blog)_
