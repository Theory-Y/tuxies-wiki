...
:::warning Install `Solaar` and `Kando` before running `install.sh`. When neither a Flatpak nor a native install is detected, the config are applied for the native app's path, but the configs would not take effect until the apps are installed!
:::
